...
#pragma mark - 长按保存到本地
- (void)longPressScrollView:(UILongPressGestureRecognizer *)longPress {
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 0) {
        NSInteger contentIndex = _scrollView.contentOffset.x/Screen_width;
        UIImageView *pressImageView = (UIImageView *)_imgViews[contentIndex];
        UIImageWriteToSavedPhotosAlbum(pressImageView.image, self, @selector(imageSavedToPhotosAlbum:didFinishSavingWithError:contextInfo:), nil);
    }
}
...
- (void)imageSavedToPhotosAlbum:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    if (!error) {
        [SVProgressHUD showImage:nil status:@"成功保存到相册"];
    }else {
