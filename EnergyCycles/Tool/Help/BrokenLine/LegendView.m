...
- (void)drawRect:(CGRect)rect {
    ...
    for(NSString *title in self.titles) {
        ...
        [title drawAtPoint:CGPointMake(COLORPADDING + PADDING, y + PADDING + 1) withAttributes:[NSDictionary dictionaryWithObjectsAndKeys:self.titlesFont,NSFontAttributeName, nil]];
        [[UIColor blackColor] set];
        [title drawAtPoint:CGPointMake(COLORPADDING + PADDING, y + PADDING) withAttributes:[NSDictionary dictionaryWithObjectsAndKeys:self.titlesFont,NSFontAttributeName, nil]];
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    CGFloat w = 0;
    for(NSString *title in self.titles) {
        CGSize s = [title sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:self.titlesFont,NSFontAttributeName, nil]];
        w = MAX(w, s.width);
    }
