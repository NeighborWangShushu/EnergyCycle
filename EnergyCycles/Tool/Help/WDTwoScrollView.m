...
@interface WDTwoScrollView ()<UIScrollViewDelegate>
...
@property (copy, nonatomic) CurrentPageBlock currentPageBlock;

@end

@implementation WDTwoScrollView
...
- (instancetype)initWithFrame:(CGRect)frame andLayout:(WDScrollViewLayout)layout
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.shouldHiddenAllOthersExceptImageContent = NO;
...
- (void)addListenerWithCurrentPageBlock:(CurrentPageBlock)block
{
...
- (void)next
{
    ...
    if (self.currentPageBlock) {
        self.currentPageBlock(self.currentIndex);
    }
}
...
- (void)configWithObject:(id)obj andButton:(UIButton*)button
{
    if ([obj isKindOfClass:[NSURL class]]) {
        ...
        [button sd_setBackgroundImageWithURL:obj forState:UIControlStateNormal placeholderImage:nil];
    }
    
    if ([obj isKindOfClass:[NSString class]]) {
        if ([obj hasPrefix:@"http"]) {
            [button sd_setBackgroundImageWithURL:[NSURL URLWithString:obj] forState:UIControlStateNormal];
        }else{
...
//- (void)setButtonFrameCenter:(UIButton*)button
//{
//    button.frame = CGRectMake(CGRectGetWidth(self.frame)+5, 5, CGRectGetWidth(self.frame)-10, CGRectGetHeight(self.frame)-10);
//}
//- (void)setButtonFrameRight:(UIButton*)button
//{
//    button.frame = CGRectMake(CGRectGetWidth(self.frame) * 2+5, 5, CGRectGetWidth(self.frame)-10, CGRectGetHeight(self.frame)-10);
//}
...
- (void)setShouldHiddenAllOthersExceptImageContent:(BOOL)shouldHiddenAllOthersExceptImageContent
{
    _shouldHiddenAllOthersExceptImageContent = shouldHiddenAllOthersExceptImageContent;
    self.titleView.hidden = shouldHiddenAllOthersExceptImageContent;
    self.titleLabel.hidden = shouldHiddenAllOthersExceptImageContent;
    self.pageControl.hidden = shouldHiddenAllOthersExceptImageContent;
}
