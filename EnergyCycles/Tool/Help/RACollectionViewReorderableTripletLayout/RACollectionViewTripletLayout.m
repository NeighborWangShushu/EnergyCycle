...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //cellSize
    CGFloat largeCellSideLength = ((_collectionViewSize.width - _insets.left - _insets.right) - _itemSpacing * 3) / 4.f;
    ...
    //列
    NSInteger list = indexPath.item % 4;
    ...
    if (list == 0) {
        lineOriginX = _insets.left;
    }else if (list == 3) {
        lineOriginX = rightSideLargeCellOriginX;
    }else {
        lineOriginX = ((_largeCellSize.width + _itemSpacing) * list) + _insets.left;
    }
    
    attribute.frame = CGRectMake(lineOriginX, lineOriginY, _largeCellSize.width, _largeCellSize.height);
