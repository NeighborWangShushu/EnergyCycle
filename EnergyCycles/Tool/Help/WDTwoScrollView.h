...
typedef void(^CurrentPageBlock)(NSInteger pageIndex);
...
@protocol WDTWOScrollViewDelegate;

@interface WDTwoScrollView : UIView

@property (weak, nonatomic) id<WDTWOScrollViewDelegate>delegate;
 ...
 *  是否隐藏所有的除图片区的控件，默认NO。
 */
@property (assign, nonatomic) BOOL shouldHiddenAllOthersExceptImageContent;
...
@protocol WDTWOScrollViewDelegate <NSObject>

@optional

- (void)wdScrollView:(WDTwoScrollView*)scrollView didClickedAtIndex:(NSInteger)index;
