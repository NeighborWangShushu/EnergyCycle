...
- (void)setMessageFrame:(UUMessageFrame *)messageFrame {
    ...
    UIImageView *headImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.btnHeadImage.bounds.size.width, self.btnHeadImage.bounds.size.height)];
    [headImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",message.strIcon]] placeholderImage:[UIImage imageNamed:@"touxiang.png"]];
    [self.btnHeadImage addSubview:headImageView];
...
//                                 placeholderImage:[UIImage imageNamed:@"headImage.jpeg"]];
    ...
    if (self.labelNum.text.length < 4) {
        self.labelNum.frame = CGRectMake(messageFrame.nameF.origin.x, messageFrame.nameF.origin.y + 3, 40, messageFrame.nameF.size.height);
        self.labelNum.textAlignment = NSTextAlignmentCenter;
    }
    ...
    switch (message.type) {
        ...
        case UUMessageTypePicture: {
            ...
            if (message.pictureUrl.length != 0) {
                [self.btnContent.backImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",message.pictureUrl]] placeholderImage:[UIImage imageNamed:@"rumo.png"]];
            }else {
        ...
        case UUMessageTypeVoice: {
            ...
            self.btnContent.second.text = [NSString stringWithFormat:@"语音"];
            
            if (message.voiceUrl.length != 0) {
                voiceURL = [NSString stringWithFormat:@"%@",message.voiceUrl];
            }else {
